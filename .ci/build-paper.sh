...
then
    ...
    cd $TRAVIS_BUILD_DIR/article && make
    ...
    mv ../eneloop.pdf .
    git init
    git add -f eneloop.pdf

    # Force push this to the [branch_name]-pdf branch on GitHub
    git status
    git -c user.name='travis' -c user.email='travis' commit -m "building the paper"
    git status
    git push -f https://$GITHUB_USER:$GITHUB_API_KEY@github.com/$TRAVIS_REPO_SLUG.git HEAD:$TRAVIS_BRANCH-pdf
